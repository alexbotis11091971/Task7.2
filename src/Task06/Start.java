    ...
    public  void StartTask() {
        ...
        for (Person6 persona : person) {// для каждого человека
            ...
            if (persona.getTimeWork()>200 && persona.getOfshor().equals("нет")){
                 ...
                 Summa= premiya.getSumma(persona);


            } else

             Summa = zarplata.getSumma(p);// сумма без учета налогов

            if (persona.getFormaOplaty().equals("почасовая")) {// если форма оплаты почасовая

                if (persona.getOfshor().equals("да")) { // если в офшоре

                    double oplata = ofshor.getPoczasow(persona, nalogi, p.getCountChasy());//должен быть вызван метод getPoczasow из Valuta
                    double tugric = ofshor.getTugric(persona, nalogi, p.getCountChasy());// иполовина суммы из valuta.getTugric
